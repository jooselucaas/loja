...
function renderProducts() {
    ...
    for (let i = currentIndex; i < currentIndex + 6 && i < products.length; i++) {
...
leftBtn.addEventListener('click', () => {
    currentIndex -= 6; // Move 6 produtos para trás
    if (currentIndex < 0) currentIndex = 0;
    renderProducts();
});

rightBtn.addEventListener('click', () => {
    currentIndex += 6; // Move 6 produtos para frente
    if (currentIndex >= products.length) currentIndex = products.length - 6; // Limita o índice
    renderProducts();
});
